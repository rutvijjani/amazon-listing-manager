...
class User(UserMixin):
    ...
    def has_amazon_connection(self):
        return AmazonConnection.get_collection().count_documents(
            {"user_id": self.id, "is_active": True}
        ) > 0

    def get_amazon_connections(self):
        cursor = AmazonConnection.get_collection().find(
            {"user_id": self.id, "is_active": True}
        ).sort("updated_at", -1)
        return [AmazonConnection(data) for data in cursor]

    def get_active_connection(self, marketplace_id=None):
        query = {"user_id": self.id, "is_active": True}
        collection = AmazonConnection.get_collection()
        if marketplace_id:
            query["marketplace_id"] = marketplace_id
            data = collection.find_one({**query, "is_selected": True})
            if not data:
                data = collection.find_one(query)
            return AmazonConnection(data) if data else None
    ...
    def get_connection_by_id(self, connection_id):
        return AmazonConnection.find_by_id(connection_id, user_id=self.id)
...
class AmazonConnection:
    ...
    @classmethod
    def find_by_id(cls, connection_id, user_id=None):
    ...
    @classmethod
    def clear_selected_for_user(cls, user_id):
    ...
    @classmethod
    def upsert_account(cls, user_id, marketplace_id, seller_id, defaults):
        """
        Create or update one seller account connection.
        Unique account match is user + marketplace + seller_id so the same
        marketplace can hold multiple seller accounts.
        """
        ...
        if existing:
            ...
            payload.setdefault("created_at", existing.get("created_at", datetime.now(UTC)))
            return cls(payload)
