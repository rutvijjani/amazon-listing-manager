...
class Invitation:
    ...
    def find_by_user(cls, user_id, marketplace_id=None, active_only=True):
        """Find a connection for the user, optionally filtered by marketplace."""
    ...
    def deactivate_selected_for_user(cls, user_id):
        """Clear selected flag for all active connections for a user."""
    ...
    def upsert_for_marketplace(cls, user_id, marketplace_id, defaults):
        """Create or update one marketplace connection for a user."""
        collection = cls.get_collection()
